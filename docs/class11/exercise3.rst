...
From tmsh or even the bash shell, there are many ways to look at traffic, here are a few suggestions : 
<CLI>
tmsh
...
From the tenant, we can see TMM processed traffic only: the network layer is virtualized by the platform of F5OS. Looking at network interfaces in the tenant shows virtual paths to each TMM:
...
All layer 2 interface statistics such as interfaces and trunks are monitored by F5OS. Details can be viewed in the F5OS webUI, viewed in the F5OS CLI, polling via SNMP or API interface calls.
...
Within the CLI, the command show interfaces interface | tab view includes more information then shown in the above webUI view which requires a large amount of screen space to view.

The CLI command can be modified to show a subset of the columns to viewed more easily in standard terminals. Experiment with adding/removing other columns to customize your own view of this example command: 
...
If troubleshooting an optical connection that has digital diagnostic monitoring (DDM) enabled, this is done at the F5OS layer, not TMOS. This views by using the the portgroup number of the interface, which will output information such as optic type, vendor along with TX/RX power levels:
...
When troubleshooting some traffic flows, tcpdump is a powerful tool. Most of the time, it is executed within the tenant which provides extended information with F5 trailers, however occasionally it may be useful to run a filtered packet capture from the F5OS command line.
...
< Need to Update to match a client IP from test traffic> 

F5OS:

r5900-1# system diagnostics tcpdump -i LAG_20G -c 10 -s1500 host 10.1.<30+X>.5

Tenant:

[root@i5000-a:Active:Standalone] config # tcpdump -nni 0.0 -c 10 -s1500 host 10.1.<30+X>.5

This filter matches a single client IP address being used by the traffic generator to help limit output. Why would the packets capture for this be different at the tenant layer than F5OS?
<<JHart - What are we looking for?  What could be different?>>
...
The F5OS CLI does contain interfaces to show log files, coupled with matching commands, allows for quick searching of some log files. Start with file list path and use tab completion to view options for directories and paths. 

r5900-1# file list path log/system | include name

Let's say we want to find a history of all F5OS boot sequences. We know the platform.log file has a BOOT-MARKER log line to search for.

This command would show the first time, but not all: how can we alter this command to see the last time the system was booted? All times? 
...
Start by navigating to the Diagnostics -> System Reports. This screen shows any currently stored qkview files by name and generation date along with logs of any uploaded to iHealth directly from the BIG-IP. To generate a new one, click the Generate QKView button. New to F5OS is the pop up box below that allows for optional collection settings:
