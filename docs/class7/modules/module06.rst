...
2. SSH to the management port of your BIG-IP. Remember the BIG-IP is a
   ...
   c. From the dump you can see you are hitting the virtual server.  Your original client IP is in the first line of
...
4. First, let’s check to see if the server is responding to HTTP on port 80. On the BIG-IP in an SSH window:
   ...
   a. Do a **<ctrl-c>** to escape out of **tcpdump**, if you are still
      in it, and use **curl** to test the server.  You should get output similar to what's below.
...
5. Turn **SNAT Automap** back on the **www_vs** virtual server
