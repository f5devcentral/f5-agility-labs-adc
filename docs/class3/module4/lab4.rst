...
#. Set client-side and server-side TCP profiles on your virtual server
   ...
   #. In some earlier version you would be required to select the
      **Advanced** menu to see the Client and Server protocol profiles.

      |image1|

   #. If you chose to use the **Advanced** menu you would see a whole array
   ...
   #. From the drop-down menus place the **tcp-wan-optimized** profile on
...
#. Visit your virtual server’s web page and refresh it several times.
...
#. Open up your **www_vs** virtual server.
   ...
   #. At the **HTTP Profile** drop down menu make sure **http** is
      selected.
...
#. Now go to **Statistics >> Module Statistics >> Local Traffic** on the
   sidebar. From the **Statistics Type** drop-down menu select
   **Profiles Summary**

|image3|

#. **Select** the View link next to the **Web Acceleration** profile
   type
...
#. You can get more detailed information on RamCache entries at the CLI
   level
...
#. Go to **Local Traffic >> Profiles >> Service >> HTTP Compression** or
   **Acceleration >> Profiles >> Web Acceleration**
   ...
   #. Create a new profile, **www-compress**, using the
...
#. Open up your **www_vs** virtual server.
   ...
   #. At the **Web Acceleration** drop-down menu select **None**
      ...
      #. *For the purpose of this lab we don’t want caching interfering
         with our response headers*

   #. At the **HTTP Compression** drop-down menu select the HTTP
...
#. Now open your virtual server’s web page and under **Content Examples
   ...
   #. Select the **View** link next to the **HTTP Compression** profile
      type
   ...
   #. On the web page under **HTTP** **Request and Response Information**
      ...
      #. Notice you no longer see the **Accept-Encoding** header in the
